  ...
  // Protected Dashboard Route (JWT Required)
    ...
    try {
      ...
      res.end(
        JSON.stringify({
          status: "success",
          message: `Welcome to the dashboard, ${decoded.username}!`,
        })
      );
    } catch (error) {
    ...
    req.on("end", async () => {
      try {
        ...
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ status: "error", message: "Invalid JSON data." })
        );
      }
    });
...
`;
        ...
        db.query(sql, values, (err, result) => {
          if (err) {
            if (err.code === "ER_DUP_ENTRY") {
              let errorMessage = "Duplicate entry error: ";
              if (err.sqlMessage.includes("employees.Email"))
                errorMessage = "Email already exists.";
              else if (err.sqlMessage.includes("employees.employee_Username"))
                errorMessage = "Username already exists.";
              else if (err.sqlMessage.includes("employees.Phone"))
                errorMessage = "Phone number already exists.";
